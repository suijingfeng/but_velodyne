...
// Euclidean Velodyne coordinate, 
//including intensity, ring number and range information
struct Point
{
  PCL_ADD_POINT4D
  ; // quad-word XYZ
...
public:
	...
	void transform(std::vector<float> DoF);
	void detectPlanes(cv::Mat projection);
	void normalizeIntensity(float min = 0.0, float max = 1.0);
	void threshold(float thresh, pcl::PointCloud<Point>& out_pc);
  ...
  static cv::Point2f projectf(const Point &pt, const cv::Mat &projection_matrix)
  {
	...
	static cv::Point project(const pcl::PointXYZI &pt, 
	...
	void project(cv::Mat projection_matrix, cv::Rect frame, 
...
POINT_CLOUD_REGISTER_POINT_STRUCT( Velodyne::Point, (float, x, x) (float, y, y) (float, z, z) (float, intensity, intensity) (uint16_t, ring, ring))
